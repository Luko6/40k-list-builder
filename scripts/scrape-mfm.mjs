...
// Manual role fixes for units the MFM doesn't badge. Keyed by datasheet slug.
const ROLE_OVERRIDES = {
  // The (Anointed) vow variant isn't listed on the MFM at all; mirror the base
  // Emperor's Champion, which is a leader.
  'emperors-champion-anointed': 'leader',
}
...
function parse(text) {
  ...
  // Each unit's LEADER/SUPPORT badge is a marker div ("children":"LEADER" + a
  // /leader.svg icon) that sits right AFTER that unit's name span (the names
  // that follow the badge are the leader's can-lead list), so a marker's role
...
// Build overrides keyed by slug. "role" now comes from the MFM LEADER/SUPPORT
// badge; fall back to any role already in the file for unmatched units.
const prev = existsSync(OUT) ? JSON.parse(readFileSync(OUT, 'utf8')) : {}
const out = {
  _comment:
    'AUTO-GENERATED by scripts/scrape-mfm.mjs from the Munitorum Field Manual RSC payload. Keyed by datasheet-name slug. Re-run after MFM updates, then npm run compile-data. "role" (leader/support) comes from the MFM badge.',
}
let roleCount = 0
...
for (const [id, role] of Object.entries(ROLE_OVERRIDES)) {
  if (!out[id]?.role) roleCount++
  out[id] = { ...(out[id] ?? {}), role }
}
writeFileSync(OUT, JSON.stringify(out, null, 2) + '\n')
console.log(`✓ wrote ${Object.keys(out).length - 1} unit overrides (${roleCount} with role) -> ${OUT}`)
